...
class OrcamentoMensalControllerUnitTest {
    ...
    class BuscarOrcamentoPorIdTest {
        ...
        void deveRetornarForbiddenQuandoUsuarioNaoEDono() throws Exception {
            ...
            outroOrcamento.setUser(outroUser);
        
            // Mock para lançar exceção quando o usuário não é dono
            when(jwtUtil.extractUserId(anyString())).thenReturn(user.getUuid());
            when(orcamentoMensalService.buscarPorId(user.getUuid(), outroOrcamento.getUuid()))
                .thenThrow(new AccessDeniedException("Acesso negado"));
        
            mockMvc.perform(get("/orcamento-mensal/" + outroOrcamento.getUuid())
                    .header("Authorization", "Bearer token_valido"))
    ...
    class CriarOrcamentoTest {
        ...
        void deveRetornarBadRequestParaCriacaoInvalida() throws Exception {
            OrcamentoMensalDTO requestDTO = new OrcamentoMensalDTO(); // DTO inválido
    ...
    class AtualizarOrcamentoTest {
        ...
        void deveRetornarForbiddenQuandoUsuarioNaoEDono() throws Exception {
            ...
            when(orcamentoMensalService.atualizarOrcamentoMensal(
                user.getUuid(), 
                outroOrcamento.getUuid(), 
                "TRANSPORTE", 
                BigDecimal.valueOf(600), 
                YearMonth.now())
            ).thenThrow(new AccessDeniedException("Acesso negado"));
        
            mockMvc.perform(put("/orcamento-mensal/" + outroOrcamento.getUuid())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(objectMapper.writeValueAsString(request))
                    .header("Authorization", "Bearer token_valido"))
                .andExpect(status().isForbidden());
    ...
    class ExcluirOrcamentoTest {
        ...
        void deveRetornarForbiddenQuandoUsuarioNaoEDono() throws Exception {
            ...
            doThrow(new AccessDeniedException("Acesso negado"))
                .when(orcamentoMensalService).excluirOrcamentoMensal(user.getUuid(), outroOrcamento.getUuid());
    
            mockMvc.perform(delete("/orcamento-mensal/" + outroOrcamento.getUuid())
                    .header("Authorization", "Bearer token_valido"))
                .andExpect(status().isForbidden());
        }
    }
